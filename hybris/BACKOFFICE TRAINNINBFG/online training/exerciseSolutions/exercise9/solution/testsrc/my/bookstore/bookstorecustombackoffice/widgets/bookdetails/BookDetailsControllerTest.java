...
{ @DeclaredInput(value = BookDetailsController.SOCKET_SELECTED_BOOK, socketType = BookModel.class),
		@DeclaredInput(value = BookDetailsController.SOCKET_ALLOW_PREVIEW, socketType = Boolean.class) })
...
public class BookDetailsControllerTest extends AbstractWidgetUnitTest<BookDetailsController>
{
	...
	public void testReceiveBookBySocketEvent()
	{
		...
		executeInputSocketEvent("selectedBook", testBook);
		// checks whether populateImgDiv was called
		Mockito.verify(getWidgetController()).populateImgDiv(testBook, baseConfig);
		// checks whether populateInfoDiv was called
		Mockito.verify(getWidgetController()).populateInfoDiv(testBook, detailsConfig);

		// checks whether addProperty method was called three times as a result of sending a book through the input socket
		Mockito.verify(getWidgetController(), Mockito.times(3)).addProperty(Mockito.any(Vlayout.class), Mockito.anyString(),
				Mockito.anyString());
	}
