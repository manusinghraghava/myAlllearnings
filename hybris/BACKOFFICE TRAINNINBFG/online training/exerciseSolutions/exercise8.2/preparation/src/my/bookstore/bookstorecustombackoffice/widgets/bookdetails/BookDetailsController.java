...
{
	// TODO exercise 8.15b
	// public static final String WIDGET_MODEL_ATTRIBUTE_CURRENT_BOOK = ...
	...
	public void handleSelectedBook(final BookModel book)
	{
		...
		// TODO exercise 8.15a: put the selected book into the Book Details widget model's 'currentBook' attribute
		// so that we have an 'attribute' for the Action to bind to,
		// Later, in the ZUL file (with help from the *-config.xml file), we will 'declare' the actual binding
		// The selectedBook can then be accessed by the Action via its ActionContext.
		// getModel().put(...)

		// In all cases, clear -- whether incoming book is null or if about to re-render
		clearImgDiv();
		clearInfoDiv();
	...
	public void handleObjectsUpdatedEvent(final CockpitEvent event)
	{
		...
		if (updatedBook != null)
		{
			...
			// TODO 8.15c Use the WIDGET_MODEL_ATTRIBUTE_CURRENT_BOOK constant you defined
			// in place of the String literal (placeholder)
			//			getModel().put("xxx", xxx);
	...
	public void handleObjectsDeletedEvent(final CockpitEvent event)
	{
		...
		if (deletedBook != null)
		{
			...
			// TODO 8.15d Use the WIDGET_MODEL_ATTRIBUTE_CURRENT_BOOK constant you defined
			// in place of the String literal (placeholder)
			//			getModel().put("xxx", xxx);
